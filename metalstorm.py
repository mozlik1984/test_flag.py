...
def fetch_musicbrainz_new_arrivals():
    ...
    for i in range(0, len(genres), chunk_size):
        ...
        # Отсекаем синглы на уровне первичного типа релиза
    ...
    for rel in all_releases:
        ...
        if band and album:
            ...
            # Защита от синглов: настоящий Album/EP должен содержать минимум 3 трека
            track_count = 0
            ...
            # Извлекаем все теги релиза, прописанные в базе
            tags_list = [t.get("name", "").lower().strip() for t in rel.get("tags", [])]
            
            # Стальной Фильтр v9.0 (Математический блок против ИИ-мусора)
            # ИИ-боты генерируют только 1 базовый тег. Настоящие металлисты пишут много поджанров.
            if len(tags_list) <= 1:
                print(f"❌ МАТЕМАТИЧЕСКИЙ БАН: {band} - {album} отклонён. Слишком мало тегов ({len(tags_list)}). Подозрение на ИИ-спам.")
                continue
            ...
            # Безопасная сборка карточки релиза строго по ТЗ
