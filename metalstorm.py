...
def verify_via_discogs(band_name, album_name):
    """ Стальной Фильтр v8.0: Жесткая проверка лейбла и цифрового формата против ИИ-мусора """
    ...
    # Сборка защищенного URL к поисковому API Discogs
    url = f"{P}{D_API}{Q}q{E}{encoded_query}{A}type{E}release"
    headers = {'User-Agent': 'MetalHubValidatorApp/5.0'}
    
    time.sleep(1.5) # Соблюдаем лимиты запросов Discogs API
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            if response.status == 200:
                ...
                if len(results) > 0:
                    ...
                    # Извлекаем список лейблов и форматов релиза
                    labels = [str(l).lower().strip() for l in first_item.get("label", [])]
                    formats = [str(f).lower().strip() for f in first_item.get("format", [])]
                    
                    # Проверяем, является ли релиз исключительно цифровым самиздатом
                    ...
                    # Если это цифровой файл БЕЗ физического носителя и БЕЗ официального лейбла — это фейк
                    if is_digital and is_not_on_label and not has_physical:
                        print(f"❌ СТАЛЬНОЙ ФИЛЬТР БАН: {band_name} - {album_name} опознан как ИИ-самиздат (Лейбл: {labels}).")
                        return False
                        
                    print(f"✅ ВЕРИФИКАЦИЯ ПРОЙДЕНА: {band_name} - {album_name} (Лейбл: {labels})")
                    return True
    except Exception as e:
        print(f"⚠️ Ошибка сети Discogs для {band_name}: {e}")
        return True # Резервный фолбэк при падении серверов, чтобы не потерять данные
        
    print(f"❌ ОТКЛОНЕН: {band_name} - {album_name} отсутствует в базах Discogs.")
    return False

def get_country_by_artist_id(artist_id, headers):
    """ Запрос кода страны исполнителя с использованием кэширования """
    if not artist_id:
        return ""
    if artist_id in artist_countries_cache:
        return artist_countries_cache[artist_id]
        
    url = f"{P}musicbrainz.org{S}ws{S}2{S}artist{S}{artist_id}{Q}fmt{E}json"
    time.sleep(1)
    try:
        req = urllib.request.Request(url, headers=headers)
        with urllib.request.urlopen(req, timeout=10) as response:
...
def fetch_musicbrainz_new_arrivals():
    ...
    for i in range(0, len(genres), chunk_size):
        ...
        # Исключаем синглы на уровне первичного типа релиза
    ...
    for rel in all_releases:
        ...
        if current_month_num:
            if rel_date and len(rel_date.split("-")) > 1:
                if rel_date.split("-") != current_month_num:
                    continue
            else:
                continue
        ...
        if band and album:
            ...
            # Защита от промо-синглов: в релизе (Album/EP) должно быть минимум 3 трека
            track_count = 0
            for medium in rel.get("media", []):
                track_count += medium.get("track-count", 0)
            if track_count > 0 and track_count < 3:
                print(f"❌ СКИПНУТ СИНГЛ: {band} - {album} содержит всего {track_count} трека(ов).")
                continue
            
            # Стальной Фильтр v8.0: Пробиваем ИИ-мусор по базам лейблов Discogs
            ...
            # Сборка карточки релиза по нашему шаблону ТЗ
    ...
    for i in range(0, len(output_text), max_len):
        ...
        # Отправка зашифрованного пакета в Telegram
